...
for (clump_p_value in clump_p_values){
  process_exposure <- function(file) {
    ...
    add_outcome <- paste("InflammatoryFactors/",file,sep = "")
    pre_result_path <- paste('Results/HNC2IF_',as.character(clump_p_value),sep = "")
    dir.create(pre_result_path)
    result_path <- paste('Results/HNC2IF_',as.character(clump_p_value),'/HNC_TO_',foldername,sep = "")
  ...
  fileslist <- list.files("InflammatoryFactors/",pattern = ".tsv")
  future_map(fileslist, function(file) {
    print(paste("processing:", file,"_", clump_p_value))
    tryCatch(process_exposure(file), error = function(e) {
